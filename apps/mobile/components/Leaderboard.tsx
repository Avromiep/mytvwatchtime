...
export function Leaderboard({
  ...
  typeOverride?: 'shows' | 'movies' | 'combined';
}) {
  const [activeTab, setActiveTab] = useState(0);
  const { tokens } = useAppearance();
  const { t } = useTranslation(['social']);
  const typeMap: Record<string, 'shows' | 'movies' | 'combined'> = {
    shows: 'shows',
    movies: 'movies',
    combined: 'combined',
  };
          ...
          {tabs.map((t, i) => (
            <View
              ...
              style={[
                ...
                activeTab === i && { backgroundColor: tokens.primary },
              ]}
            >
              <T
                variant="caption"
                style={{ color: activeTab === i ? tokens.primaryForeground : tokens.textMuted }}
              ...
              {i < tabs.length - 1 ? (
                <T
                  ...
                  onPress={() => setActiveTab(Math.min(tabs.length - 1, i + 1))}
                >
                  ›
                </T>
              ) : null}
      ...
      <View style={styles.headerRow}>
        ...
        >
          <Ionicons
            ...
            size={24}
            color={page <= 1 ? tokens.textDim : tokens.textMuted}
          />
        </Pressable>
        <T variant="micro" muted>
          {showPager ? `${page} / ${totalPages}` : ''}
        </T>
        <Pressable
        ...
        >
          <Ionicons
            ...
            size={24}
            color={page >= totalPages ? tokens.textDim : tokens.textMuted}
          />
        </Pressable>
      </View>
  ...
  return (
    <Pressable
      ...
      style={[
        ...
        highlight && { backgroundColor: tokens.surfaceAlt },
      ]}
    >
      <View style={styles.posCol}>
        {medal ? (
          <T variant="h2">{medal}</T>
        ) : (
          <T variant="h2" style={{ color: tokens.textMuted }}>
            {entry.position}
          </T>
        )}
      </View>
