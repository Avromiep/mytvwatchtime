...
function toAttached(item: MediaCardDto): AttachedMedia {
  if (item.type === MediaType.SHOW) {
    ...
    return {
      ...
      mediaId: s.id,
      title: s.title,
      posterUrl: s.images?.poster ?? null,
  ...
  return {
    ...
    mediaId: m.id,
    title: m.title,
    posterUrl: m.images?.poster ?? null,
...
export function MediaPickerModal({
  ...
  const results =
    debounced.length > 1 ? (search.data?.pages ?? []).flatMap((p) => p.items ?? []) : [];
                  ...
                  {shareableLists.map((l) => (
                    ...
                    >
                      ...
                      <View style={styles.listCardContent}>
                        <T
                          variant="micro"
                          style={{ color: tokens.mediaText, fontWeight: '700' }}
                          numberOfLines={2}
                        >
                          {l.title}
                        </T>
