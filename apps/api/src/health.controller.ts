...
export class HealthController {
  ...
  async getPublicFlags() {
    ...
    result.comment_translation = caps.commentTranslation;
  ...
  @Post('push/relay')
  async pushRelay(
    @Body() body: { token: string; title: string; body?: string; data?: Record<string, unknown> },
  ) {
    ...
    if (!body?.token || !body?.title)
      throw new HttpException('token and title required', HttpStatus.BAD_REQUEST);
    ...
    if (current > rateLimit) {
      throw new HttpException(
        `Rate limit exceeded (${rateLimit}/${windowMin}min)`,
    ...
    if (!expoToken)
      throw new HttpException('Push not configured on this server', HttpStatus.SERVICE_UNAVAILABLE);
