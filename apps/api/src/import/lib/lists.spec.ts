  ...
  it('recovers an unnamed uuid list title from the collection blob (s_key → name)', () => {
    const { lists } = normalizeLists([
      row({
        s_key: 'collection',
        type: '',
        lists: '[map[created_at:1.7e+09 description:<nil> fanart:[https://a/x.jpg https://a/y.jpg] is_public:false name:Sci-fi order:<nil> posters:[https://a/p.jpg] s_key:c64b6ccd-688f-4c0c-bf8d-2176c07cf5cf type:list updated_at:1.7e+09 user_id:6.2e+07]]',
      }),
  ...
  it('prefers the row name over the collection blob name', () => {
    const { lists } = normalizeLists([
      row({
        s_key: 'collection',
        type: '',
        lists: '[map[is_public:true name:Blob Name s_key:f981c085-29f1-41b7-a6f8-fa5c3a267866 type:list]]',
      }),
      row({ s_key: 'f981c085-29f1-41b7-a6f8-fa5c3a267866', type: 'list', name: 'comedy', objects: '[]' }),
    ]);
...
describe('buildMovieUuidNameMap', () => {
  it('maps movie uuids to names from any file carrying both columns (first name wins)', () => {
    const map = buildMovieUuidNameMap([
      { filename: 'tracking-prod-records.csv', rows: [row({ uuid: 'u-1', movie_name: 'Arrival' })] },
      { filename: 'ratings-live-votes.csv', rows: [row({ uuid: 'u-1', movie_name: 'Duplicate' }), row({ uuid: 'u-2', movie_name: '聲の形' })] },
      { filename: 'comments-prod-comments.csv', rows: [row({ id: '1', text: 'ignored' })] },
    ]);
  ...
  it('ignores <nil>/empty values', () => {
    const map = buildMovieUuidNameMap([
      { filename: 'tracking-prod-records.csv', rows: [row({ uuid: '<nil>', movie_name: 'X' }), row({ uuid: 'u-3', movie_name: '' })] },
    ]);
    expect(map.size).toBe(0);
  });
});
