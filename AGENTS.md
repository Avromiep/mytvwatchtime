...
- Non-English base titles (rows whose base/override was written in the wrong language — old contamination that makes English users see foreign titles): Metadata Health `nonEnglishBase` stat (`title_locale != 'en'` or missing `titles.en`) + `POST /admin/repair-non-english-base/run` (`repairNonEnglishBase`): clears `metadataRefreshedAt` and re-runs the standard hydration (TMDB id first, TVDB fallback) — `persistShow/persistMovie` rewrite the English base and overwrite the wrong value in the `en` slot. User data untouched.
